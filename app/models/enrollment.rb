...
=begin
  ...
  user_params = { :name => "Ruby Chrissandy", :nim => "hah", 
        :email => "someone@yahoo.com", :password => "ruby1234r", 
...
# creating student has to be by using this method
  ...
  def Enrollment.create_user_with_enrollment( user_params, enrollment_params , school_id, role_id )
    ...
    @user = school.find_student_by_nim( user_params[:nim] )
    if   ( not @user.nil? )
      @user.update_with_params( user_params, password )
      return Enrollment.find(:first, :conditions => {
        :user_id => @user.id,
        :school_id => school_id
        })
    end
